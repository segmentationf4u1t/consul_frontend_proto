...
type PanelPoint = { id: number; timestamp: string; kolejka: number }
type PanelsPage = { items: PanelPoint[]; nextCursor: { ts: string; id: number } | null }
...
/**
 * Heatmap of queue size (kolejka) per hour × day from /historical/panels.
 */
export function QueueHeatmap({ className, days = 14 }: QueueHeatmapProps) {
  const [loading, setLoading] = useState(true)
  const [refreshing, setRefreshing] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [points, setPoints] = useState<Array<{ ts: number; timestamp: string; value: number }>>([])
  const firstLoadDoneRef = useRef(false)
  const [isDark, setIsDark] = useState(false)
  // Simple white → red scale (reverted per request)

  const cutoffMs = useMemo(() => Date.now() - days * 24 * 3600_000, [days])
  ...
  const fetchAll = async () => {
    try {
      ...
      let acc: Array<{ ts: number; timestamp: string; value: number }> = []
      let pageCount = 0
      ...
      do {
        ...
        const mapped = page.items
          .map((r) => {
            ...
            return { ts, timestamp: r.timestamp, value: Number(r.kolejka ?? 0) }
          })
          .filter((x): x is { ts: number; timestamp: string; value: number } => !!x)
      ...
      setPoints(acc.sort((a, b) => a.ts - b.ts))
    } catch (e: any) {
  ...
  const { rows, dayLabels, globalMin, globalMax } = useMemo(() => {
    ...
    for (const dateKey of limitedDates) {
      ...
      for (let h = 0; h < 24; h++) {
        ...
        const val = agg ? Math.round(agg.sum / Math.max(1, agg.count)) : -1
        row.push(val)
        if (val >= 0) {
  ...
  if (error) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Heatmap kolejki</CardTitle>
        </CardHeader>
        <CardContent>
